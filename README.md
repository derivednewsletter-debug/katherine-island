...
- **Farm HUD** — a 🌱 Farm panel lists every planted crop across the island with its stage, biome, and a live growth progress bar; ready crops glow and a badge counts them, and a 🧭 Fly button glides the camera straight to each patch.
- **Seed economy & exotic crops** — every crop needs **seeds**, sold at the shop as repeatable seed packs (planting consumes one). The shop's ✨ Exotic section unlocks the rare **🌙 night flower** — peaks only, and it only grows during **night phases** — then its own seeds go on sale.
- **Weather-aware farming** — rare **rain showers** (falling streaks, dimmed storm sky, procedural patter + distant thunder) sweep the island off the game clock and **double crop growth while they last**; the Farm HUD flags 🌧️ 2× showers live. If the island goes too long without rain, **jungle fruit wilts** and slowly loses progress (🥀 badge) until the next shower revives it — tying the farm to the day-night clock and the weather.
